...
reinforce board p = 
    --trace ("Reinforcing " ++ show board) $
    --                trace ("Resulting in " ++ show finalboard) 
                    finalboard
   where
        finalboard = board { 
...
--        trace ("scenario 1 " ++ show board ++ " " ++ show p ++ " " ++ show isFirstMove) 
        Node GameState {
            currentPlayer = p,
            currentMoves = [Pass],
            currentBoard = board
    ...
    | null possibleMoves && isFirstMove = 
        -- trace ("scenario 2 " ++ show board ++ " " ++ show p ++ " " ++ show isFirstMove) 
        Node GameState {
            currentPlayer = p,
            currentMoves = [Pass],
            currentBoard = board
    ...
    | otherwise = 
        -- trace ("scenario 3 " ++ show board ++ " " ++ show p ++ " " ++ show isFirstMove) 
        Node GameState {
            currentPlayer = p,
...
main = do
    ...
    print t2
    where
        tree = gameTree test3x3BoardC (Player 1) True
