  ...
  (if first-move
      ...
      (cons (list nil
		  (game-tree (add-new-dice board player (1- spare-dice))
			     (mod (1+ player) *num-players*)    ;; Switch players
			     0                                  ;; Starting off with no spare dice
			     t))                                ;; New player's first turn
	    moves)))
			    ...
			    (list
			     (list
			      ...
			      (game-tree (board-attack board cur-player src dst (dice src))
					 cur-player                     ;; Same player continues
					 (+ spare-dice (dice dst))      ;; Accumulate the dice conquered
					 nil)))))                       ;; Not their first turn
			(neighbors src))))
	    (loop for n below *board-hexnum*
		  collect n))))
 
(defun just-attacking-moves (board cur-player)
  (labels ((player (pos)
	     (car (aref board pos)))
	   (dice (pos)
	     (cadr (aref board pos))))
